if (!sqlParser) {
  ...
  Sql.prototype.travelFunctionCall = function(ast) {
    ...
    this.append(')', true);
    if (ast.overOpt) this.travel(ast.overOpt);
  };
  ...
  Sql.prototype.travelCastInterval = function(ast) {
    this.appendKeyword('cast');
    this.append('(', true, true);
    this.travel(ast.expr);
    this.appendKeyword('as');
    this.appendKeyword('interval');
    this.travel(ast.castTo);
    this.append(')', true);
  };
  ...
  Sql.prototype.travelOver = function(ast) {
    this.appendKeyword('over');
    this.append('(', true, true);
    if (ast.partitionBy) this.travel(ast.partitionBy);
    if (ast.orderBy) this.travel(ast.orderBy);
    this.append(')', true);
  };
  ...
  Sql.prototype.travelUnion = function(ast) {
    ...
    this.travel(ast.right);
    if (ast.orderBy) this.travel(ast.orderBy);
    if (ast.limit) this.travel(ast.limit);
  };
